...
const Sidebar = ({ currentPath = "/dashboard" }: { currentPath: string }) => {
  ...
  return (
    <div className="fixed left-0 top-0 bg-gray-900 text-white w-64 min-h-screen p-6 z-7">
      <div className="mb-8 ">
        ...
        {navigation.map((item, key) => {
          ...
          return (
            <Link
              href={item.href}
              key={key}
              className={`${isActive ? "bg-purple-100 text-gray-800" : "text-gray-300 hover:bg-gray-800"} px-3 flex items-center space-x-3 py-2 rounded-lg`}
            >
              <IconComponent />
